...
def main() -> None:
    ...
    report = {
        ...
        "present_decodable_originals": len(assets) - len([x for x in failures if "original" in x]),
        "present_decodable_thumbnails": len(assets) - len([x for x in failures if "thumb" in x]),
        "present_decodable_medium": len(assets) - len([x for x in failures if "medium" in x]),
    ...
    out = ROOT / "QA" / "photo_integrity.json"
