...
PAIRED_COMPARISON_OUTPUT = EVIDENCE_ROOT / "paired_comparison.json"
PERFORMANCE_SUMMARY_OUTPUT = EVIDENCE_ROOT / "performance_summary.log"
FINDING_MATRIX = EVIDENCE_ROOT / "finding_matrix.json"
SAME_HOST_WEBKIT_OUTPUT = EVIDENCE_ROOT / "same_host_webkit_control.json"
...
def same_host_webkit_control(playwright, candidate_url: str, candidate_source: dict) -> dict:
    ...
    try:
        ...
        for width, height, touch in WEBKIT_CONTROL_MATRIX:
            base_root = SCREENSHOTS_ROOT / "same_host_webkit/base" / str(width)
            candidate_root = SCREENSHOTS_ROOT / "same_host_webkit/candidate" / str(width)
...
def run_workflow(page, shots: Path, touch: bool = False, url: str = MODULAR_URL) -> dict:
    ...
    page.evaluate("document.documentElement.style.zoom='2'")
    page.wait_for_timeout(120)
    checks["screenshots"].append(screenshot(page, shots, "reflow_720_en_dark_200"))
    page.evaluate("document.documentElement.style.zoom=''")
    page.set_viewport_size({"width": 1440, "height": 900})
    return checks
...
def matrix_case(playwright, browser_name: str, width: int, height: int, touch: bool, phase: str, shots: Path, url: str = MODULAR_URL, clean_root: Path | None = None) -> dict:
    browser = getattr(playwright, browser_name).launch(headless=True, timeout=90000)
    context = browser.new_context(viewport={"width": width, "height": height}, has_touch=touch, is_mobile=width <= 390)
    page = context.new_page()
    errors, console_errors, failed = [], [], []
    errors_for(page, errors, console_errors, failed)
    row = {"browser": browser_name, "viewport": {"width": width, "height": height}, "touch": touch, "errors": errors, "console_errors": console_errors, "failed_requests": failed}
    try:
        wait_ready(page, url)
        row["checks"] = js_checks(page)
        overall_name = f"{browser_name}_{width}_overall"
        row["visual"] = {"screenshot": screenshot(page, shots, overall_name)}
        if clean_root:
            row["visual"]["map_visual_integrity"] = map_visual_integrity(page, shots, overall_name, clean_root / f"{overall_name}.png")
        else:
            row["visual"]["map_visual_integrity"] = {"status": "PASS", "mode": "baseline_capture"}
        page.locator('[data-route="A2"]').click()
        page.wait_for_timeout(100)
        page.locator('[data-tab="details"]').click()
        page.wait_for_timeout(100)
        row["checks"]["detail"] = js_checks(page)
        detail_name = f"{browser_name}_{width}_detail"
        row["visual"]["detail_screenshot"] = screenshot(page, shots, detail_name)
        if clean_root:
            row["visual"]["detail_map_visual_integrity"] = map_visual_integrity(page, shots, detail_name, clean_root / f"{detail_name}.png")
        else:
            row["visual"]["detail_map_visual_integrity"] = {"status": "PASS", "mode": "baseline_capture"}
        row["status"] = "PASS" if (
            not errors
            and not console_errors
            and row["checks"]["canvas"] == 1
            and row["checks"]["overflow"] == 0
            and all(item["hit"] for item in row["checks"]["occluded"])
            and row["visual"]["map_visual_integrity"].get("status") == "PASS"
            and row["visual"]["detail_map_visual_integrity"].get("status") == "PASS"
        ) else "FAIL"
    except Exception as error:
        ...
        browser.close()
    return row
def capture_baseline_matrix(playwright) -> dict:
    temporary = tempfile.mkdtemp(prefix="gate5-r11-baseline-", dir=ROOT.parent)
    worktree = Path(temporary) / "source"
    server = None
    try:
        added = subprocess.run(["git", "worktree", "add", "--detach", "--quiet", str(worktree), BASE_REVISION], cwd=ROOT, capture_output=True, text=True)
        if added.returncode:
            raise RuntimeError(f"could not create exact-main baseline worktree: {added.stderr.strip()}")
        build = worktree / ".build"
        built = subprocess.run([sys.executable, str(worktree / "scripts/build_map_first.py"), "--output-dir", str(build)], cwd=worktree, capture_output=True, text=True, timeout=120)
        if built.returncode:
            raise RuntimeError(f"exact-main baseline build failed: {built.stderr[-1200:]}")
        port = free_local_port()
        url = f"http://127.0.0.1:{port}/index.html"
        server = subprocess.Popen([sys.executable, str(worktree / "scripts/serve_map.py"), "--port", str(port), "--directory", str(build / "modular")], cwd=worktree, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
        wait_for_server(url, server)
        binding = source_binding(BASE_REVISION, worktree)
        rows = []
        for browser_name, width, height, touch in MATRIX:
            rows.append(matrix_case(playwright, browser_name, width, height, touch, "baseline", SCREENSHOTS_ROOT / "baseline", url=url))
        browser = playwright.chromium.launch(headless=True, timeout=90000)
        ...
        try:
            ...
            errors_for(page, errors, console_errors, failed)
            wait_ready(page, url)
            workflow = run_workflow(page, SCREENSHOTS_ROOT / "baseline", url=url)
        finally:
            page.close()
            context.close()
            browser.close()
        return {"status": "PASS" if all(row.get("status") == "PASS" for row in rows) else "FAIL", "revision": BASE_REVISION, "source_binding": binding, "browser_matrix": rows, "workflow": workflow}
    except Exception as error:
        return {"status": "VERIFY_REQUIRED", "revision": BASE_REVISION, "failures": [str(error)], "browser_matrix": []}
    finally:
        _terminate_server(server)
        subprocess.run(["git", "worktree", "remove", "--force", str(worktree)], cwd=ROOT, capture_output=True, text=True)
        try:
...
def write_review_index(report: dict) -> None:
    entries = []
    for path in sorted(SCREENSHOTS_ROOT.rglob("*.png")) if SCREENSHOTS_ROOT.is_dir() else []:
        relative = path.relative_to(EVIDENCE_ROOT)
        stem = path.stem
        browser = next((name for name in ("chromium", "firefox", "webkit") if stem.startswith(name + "_")), "chromium")
        viewport_match = next((part for part in stem.split("_") if part.isdigit()), None)
        entries.append({
            "path": str(relative),
            "source_revision": report.get("candidate_head") if "candidate" in relative.parts else BASE_REVISION,
            "browser": browser,
        "viewport": {"width": int(viewport_match) if viewport_match else None, "height": {1440: 900, 834: 1112, 390: 844, 720: 900}.get(int(viewport_match) if viewport_match else None)},
            "language": "en" if "_en_" in stem else "ko",
            "theme": "dark" if stem.endswith("_dark") else "light",
            "state": "filtered" if "filtered" in stem else "detail" if "detail" in stem else "overall" if "overall" in stem else "reflow_200" if "reflow" in stem else "loading_or_recovery",
        })
...
def main() -> int:
    ...
    with sync_playwright() as playwright:
        ...
        if args.phase == "candidate":
            baseline_capture = capture_baseline_matrix(playwright)
            report["baseline_capture"] = {"status": baseline_capture.get("status"), "revision": baseline_capture.get("revision"), "source_binding": baseline_capture.get("source_binding"), "failures": baseline_capture.get("failures", [])}
            report["baseline_browser_matrix"] = baseline_capture.get("browser_matrix", [])
            report["baseline_workflow"] = baseline_capture.get("workflow", {})
            if baseline_capture.get("status") != "PASS":
                report["verify_required"].append("exact current-main baseline browser capture was not complete")
        if args.phase == "candidate":
            report["same_host_webkit_control"] = same_host_webkit_control(playwright, MODULAR_URL, binding)
        for browser_name,width,height,touch in MATRIX:
            report["browser_matrix"].append(matrix_case(playwright,browser_name,width,height,touch,args.phase,shot_root,clean_root=SCREENSHOTS_ROOT/"baseline" if args.phase == "candidate" else None))
    ...
    report["verify_required"]=["E2 independent multimodal visual/usability acceptance","native Safari on macOS preflight/evidence","real physical iPhone/iPad field evidence and memory claim"]
    report["failures"]=[f"browser {r['browser']} {r['viewport']['width']} status=FAIL" for r in report["browser_matrix"] if r.get("status")=="FAIL"]
    if any(r.get("status") == "UNVERIFIED" for r in report["browser_matrix"]): report["verify_required"].append("one or more automated browser matrix cases were unverified")
