...
report = {"browser": "Chromium real MapLibre render", "expected_states": 1240, "rendered_states": len(rows), "failed_states": [row for row in rows if row["failures"]], "page_errors": errors, "status": "PASS" if len(rows) == 1240 and not errors and not any(row["failures"] for row in rows) else "FAIL", "rows": rows}
