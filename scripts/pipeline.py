...
GATE5_TIMEOUT_RATIONALE = (
    "Gate 5 runs the complete browser/accessibility matrix, same-host WebKit control, "
    "and two order-balanced eight-sample performance blocks. R11 exceeded the generic "
    "300s component bound, so R12 gives this objective a finite 900s budget while "
    "leaving all ordinary components on the 300s bound."
)
...
COMPONENTS = (
    ...
    ("gate5_field_quality", "scripts/qa_gate5_field_quality.py", "QA/project_os_verify/gate5_r12/candidate.json"),
)
...
def validate_gate5_execution(
    ...
    """Validate Gate 5 process completion before considering its report status."""
    if returncode != 0:
        if returncode == 124:
    ...
    if payload.get("candidate_head") != expected_revision:
        return {"status": "FAIL", "process_completed": False, "acceptance_status": payload.get("status"), "execution_status": "MISMATCH", "status_reason": "Gate 5 evidence report is bound to a different candidate head."}
    ...
    return {
        ...
        "status_reason": "Gate 5 subprocess exited 0 and produced a fresh exact-candidate-bound report.",
        "report": payload,
    }


def prepare_gate5_output(path: Path) -> int:
    """Remove any prior Gate 5 report and return this invocation's start marker."""
...
def qualification_decision(tests: list[dict]) -> dict:
    """Apply canonical qualification semantics, including Gate 5 execution."""
    decisive_failures = [test for test in tests if test["name"] != "gate5_field_quality" and test["status"] != "PASS"]
    ...
    return {"status": "PASS", "external_verify_required": [test["name"] for test in tests if test["status"] == "VERIFY_REQUIRED"], "reason": None}
...
def run_qualification(expected_revision: str | None = None, require_clean: bool = False) -> dict:
    ...
    try:
        ...
        for name, script, output in COMPONENTS:
            ...
            if name == "gate5_field_quality":
                gate5_validation = validate_gate5_execution(output_path, code, report["candidate_head"], invocation_started_ns)
                status = gate5_validation["status"]
            else:
                status = evidence_status(output_path, code)
            command_text = f"python3 {script}" + (" --runtime-only" if name == "photo_integrity" else "") + (f" --mode browser --output {output}" if name == "gate4_runtime" else "") + (f" --expected-revision {report['candidate_head']} --output {output}" if name == "gate5_field_quality" else "")
            if name == "maplibre_security":
