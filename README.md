...
Before Project OS GO, every Fabric candidate push to `codex/**` is independently qualified by `.github/workflows/candidate-qualification.yml` on a fresh GitHub-hosted `ubuntu-latest` runner. That lane checks out the exact triggering `${{ github.sha }}`, installs the supported Python/QA runtime plus Chromium, Firefox, and WebKit, runs `python3 scripts/pipeline.py qualify --revision "$GITHUB_SHA"`, runs repository unit/contract checks, and uploads the current qualification evidence and browser screenshots even when a check fails. It has `contents: read` only and never assembles or deploys Pages. The main-only workflow remains the production release qualification and deployment authority.
