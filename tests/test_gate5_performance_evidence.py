...
def samples(values, environment=None):
    return {
        ...
        "source_binding": source_binding("candidate"),
...
class Gate5PerformanceEvidenceTests(unittest.TestCase):
    ...
    def test_order_sensitive_regression_requires_verification_with_pair_evidence(self):
        baseline = samples([100, 110, 120])
        candidate = samples([160, 105, 170])
        paired = [
            {"pair": 1, "order": ["candidate", "baseline"], "candidate": {"cold_milestones": {"first_actionable_state": 160}}, "baseline": {"cold_milestones": {"first_actionable_state": 100}}},
            {"pair": 2, "order": ["baseline", "candidate"], "candidate": {"cold_milestones": {"first_actionable_state": 105}}, "baseline": {"cold_milestones": {"first_actionable_state": 110}}},
            {"pair": 3, "order": ["candidate", "baseline"], "candidate": {"cold_milestones": {"first_actionable_state": 170}}, "baseline": {"cold_milestones": {"first_actionable_state": 120}}},
        ]
        ...
        self.assertTrue(row["pair_evidence"]["ambiguous_direction"])

    def test_source_binding_revision_mismatch_is_verify_required(self):
        baseline = samples([100, 100, 100])
        candidate = samples([100, 100, 100])
